...
class ProductController extends AbstractController
{
    ...
    public function new(Request $request,\App\Repository\Admin\CategoryRepository $categoryRepository): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $this->addFlash('info','Kayıt Ekleme Başarılı.');
            $em->persist($product);
            $em->flush();
     ...
     */
    public function edit(Request $request, Product $product,\App\Repository\Admin\CategoryRepository $categoryRepository): Response
    {
        $catlist = $categoryRepository->findAll();
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $this->addFlash('info','Kayıt Güncelleme Başarılı.');
    ...
    public function iupdate(Request $request,$id, Product $product): Response
    {
        ...
        $form->handleRequest($request);
            ...
            try {
                $file->move(
                    $this->getParameter('images_directory'),//services.yamldaki resim yolu
                    $fileName
                );
    ...
    public function delete(Request $request, Product $product): Response
    {
        if ($this->isCsrfTokenValid('delete'.$product->getId(), $request->request->get('_token'))) {
            $em = $this->getDoctrine()->getManager();
            $this->addFlash('warning','Kayıt Silme Başarılı.');
            $em->remove($product);
            $em->flush();
        }
