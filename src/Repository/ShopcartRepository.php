...
class ShopcartRepository extends ServiceEntityRepository
{
    ...
    public function getUserShopCart($userid): array
    {
        ...
        SELECT p.title,p.sprice , s.quantity,s.productid,s.userid, (p.sprice*s.quantity)as total FROM App\Entity\Shopcart s, App\Entity\Admin\Product p WHERE s.productid = p.id and s.userid=:userid
    ...
    //  ÜRÜNLERİN TOPLAMINI HESAPLAYAN FONKSİYON
    //YÜKSEL HOCA 2 TANE DAHA FONKSİYON YAZDI FAKAT KULLANMADIĞIMIZ İÇİN YAZMADIM
    public function getUserShopCartTotal($userid): float
    {
        $em = $this->getEntityManager();

        //AŞAĞIDAK KODLAR ENTİTY YARDIMIYLA GETİRİLEN VERİLERDİR.
        $query = $em->createQuery('
    ...
    //SEPETTEKİ ELEMENLARIN TOPLAM SAYISI
